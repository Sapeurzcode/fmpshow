...
function preload() {
    for (let i = 0; i < totalpages_main; i++) {
        let page = loadImage('fmpshow/images/page' + i + '.JPG');
        pages_main.push(page);
        //    pages_main[i] = loadImage('images/page'+ i + '.JPG');
    }
      
    for (let i = 0; i < totalpages_branch7; i++) { // can add more pages later
        pages_7a.push(loadImage('fmpshow/images/page7a_' + i + '.JPG'));
        pages_7b.push(loadImage('fmpshow/images/page7b_' + i + '.JPG'));
    }
    

    for (let i = 0; i < totalpages_branch19; i++) { // can add more pages later
        pages_19a.push(loadImage('fmpshow/images/page19a_' + i + '.JPG'));
        pages_19b.push(loadImage('fmpshow/images/page19b_' + i + '.JPG'));
      }
   
    for (let i = 0; i < totalpages_branch39a; i++) { // can add more pages later
        pages_39a.push(loadImage('fmpshow/images/page39a_' + i + '.JPG'));
    }
    for (let i = 0; i < totalpages_branch39b; i++) { // can add more pages later
        pages_39b.push(loadImage('fmpshow/images/page39b_' + i + '.JPG'));
    }

    //MUSIC
    spacesinbetween = loadSound('fmpshow/music/spacesinbetween.mp3');   //music to be played in the middle ground
    dearlittlebrother = loadSound('fmpshow/music/dearlittlebrother.mp3');   //music to be played in the real world
    calm = loadSound('fmpshow/music/calm.mp3')  //music to be playes when mc enters husbands dream
}
